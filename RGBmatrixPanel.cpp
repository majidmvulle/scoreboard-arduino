...
RGBmatrixPanel::RGBmatrixPanel(uint8_t a, uint8_t b, uint8_t c, uint8_t d, uint8_t e,
                               uint8_t clk, uint8_t lat, uint8_t oe,
                               boolean dbuf, uint8_t width
#if defined(ARDUINO_ARCH_SAMD) || defined(ARDUINO_ARCH_ESP32)
                               ,
  ...
  init(32, a, b, c, clk, lat, oe, dbuf, width
#if defined(ARDUINO_ARCH_SAMD) || defined(ARDUINO_ARCH_ESP32)
       ,
       pinlist
#endif
  ...
  addrdport = portOutputRegister(digitalPinToPort(d));
  addrdmask = digitalPinToBitMask(d);
  // Init a few extra 64-specific elements:
  _e = e;
...
void RGBmatrixPanel::display_image(int16_t x, int16_t y, const uint16_t bitmap[], int16_t w, int16_t h)
{
  Adafruit_GFX::drawRGBBitmap(x,y,bitmap,w,h);
}

void RGBmatrixPanel::setFont(const GFXfont * f)
{
